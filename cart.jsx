...
const Products = (props) => {
  ...
  let list = items.map((item, index) => {
    //incase png not exist || want to use random pictures
    //let n = index + 1049;
    ...
    return (
      <li key={index}>
        ...
        <Button variant="secondary" type="submit" name={item.name} onClick={addToCart}>
  ...
  let cartList = cart.map((item, index) => {
    return (
      <Card key={index}>
        <Card.Header>
          <Button variant="secondary" type="submit" eventkey={1 + index} onClick={() => deleteCartItem(index)}>
  ...
  return (
    <Container>
      <Row>
        ...
        <Col>
          <h4>CheckOut </h4>
          <Button onClick={checkOut} variant="secondary">CheckOut $ {finalList().total}</Button>
